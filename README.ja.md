...
**SparkArc Studio** は、マルチエージェント（Agent）による自律型クリエイティブパイプラインです：ひらめきの火花を、完成した「動く物語世界」へ**コンパイル**します——小説・脚本の執筆から、Web 演出 / Unity エンジン演出の駆動までを一気通貫で実現します。

本作は、**「インスピレーション → 設定 → テンポ（ビート） → プロット大綱（アウトライン） → 執筆 → 検証 → リリース → 共有 → 演出」** の全ライフサイクルを繋ぎ、クリエイターに強力な生産性ツールスイートを提供します。

> 📊 **エンジニアリング概要**: 登録 Agent 9 体（委派可能な専門家 7 体 + システム内サービス 2 つ）· 統一ファサード経由ツール 56 個 · 三モーダル・プロンプトプロトコル · 安定プレフィックス・キャッシュ設計 · 自動化テストファイル 170+ · 5 プラットフォームクライアント · 4 言語 UI · MCP リモート接続
>
> ⚡ **2 ステップで起動**: `git clone https://github.com/1deaaa/spark-arc-studio && cd spark-arc-studio && docker compose up -d --build` → `http://localhost:7788` を開く
>
...
* **演出アセット AI 生成（リリース済み）**: 背景、キャラクター立ち絵、シーンイラストのアップロードとオンライン生成に対応。**スタイル / シーン / キャラクター / 連続性の 4 種類の一貫性参照画像**で画風を固定し、同一作品内で立ち絵と背景のスタイルを統一します。アップロードした立ち絵は自動で背景除去でき、複数ベンダーの画像生成モデル（OpenAI / Gemini / xAI など）を統合的にサポートします。
* **没入型演出プレイヤー**: Three.js によるフルスクリーンの雰囲気シェーダーと、フレームレートに応じた自動グレードダウンを搭載。小説モードでは純粋リーダー（めくり / スクロール、文字サイズ、読書進捗）に自動で切り替わります。演出アセットはバージョンスナップショットと共に配信されます。
* **公開共有コンテンツセーフティ**: 作品の公開前に AI による分割並行レビュー（Critic コンプライアンスモード）が実行され、不合格の場合は公開できません。管理者は公開共有ポリシーを全体制御できます。
* **計画中の機能**:

> 1. Scriptwriter のカスタムサブエージェント（日常シナリオ執筆者・アイテム設定執筆者など）
> 2. ユーザー定義のデータ構造と、Agent が生成する編集コンポーネント（LUI / GEN-UI 方向）
> 3. スタイルクローンの自動チューリングテスト・ループ（評価ルーブリックは分析プロンプトに内蔵済み）
...
* [品質エンジニアリング](#品質エンジニアリング)
  * [インタラクティブ脚本形式（ARC）](#arcインタラクティブ脚本形式の例)
  * [作品メモリプール](#作品メモリプール)
  * [小説モード](#小説モード)
  * [エンジニアリング品質ガードレール](#エンジニアリング品質ガードレール)
...
> 💡 **多言語対応（国際化）**: エージェント登録簿（[registry.py](server/agents/registry.py)）は `zh-CN` / `en-US` / `ja-JP` / `ko-KR` の 4 言語をネイティブサポートしています。フロントエンドは i18n マッピングを使用し、バックエンドはリクエストのロケールに基づいて `_resolve_i18n_field` を呼び出すことで、対応する翻訳フィールドを抽出します。新しい言語を追加する場合は、各エージェントのエントリに翻訳グループを追加するだけです。
...
* **スタイルエージェント (Style)**（文体クローンサブクラスター）:
  ...
  * **クラスター構成**: シリアル分析を統合的に担う **UnifiedStyleAnalyzer** と、スタイルプロファイルに基づく Q&A 対話を行う **StyleChatAgent** の 2 つが連携します。詳細は[文体クローンクラスター](#文体クローンクラスター)セクションをご参照ください。
...
> 📗 実行時の割り振りロジック、`pipeline_system` 記述上の制約、ツール参照（Reference）自動注入機能、および新規エージェント追加時のチェックリストは、[アーキテクチャ詳細ドキュメント §2](docs/project/architecture.md#2-agent-统一调用管线) および [AGENTS.md §4.5](AGENTS.md) をご参照ください。

#### 文体クローンクラスター

SparkArc の中で最も技術的深度が高いモジュールです。**UnifiedStyleAnalyzer（統合アナライザー）**によるシリアルリレー分析と **StyleChatAgent（スタイルプロファイル Q&A）**で構成され、人間のクリエイターが持つ繊細な文体を捉えて**実行可能なスタイルプロファイル**を構築します。これは後続の生成を制御し、AI 独特の頻出表現を抑制するために使用されます。

* **シリアル分析**: 長編小説を 30k トークン単位に細分化し、各ブロックで **5 つの次元の完全分析**を行います（思考と認知フィンガープリント / 言語感覚 / 感情処理 / 感覚と注意 / 対人フィールド）。ブロック間でストーリー概要を引き渡して文脈を維持し、断片的な検索では失われがちな文脈の欠落を回避します。
* **実行可能な成果物**: すべての結論は「観察ではなく指令」である必要があり、マスキング済みの短い用例を根拠として添えます。最終サマリーでは、標徴的な特徴、定常 vs 高揚語体、**作者が回避すべきネガティブ制約（タブー一覧）**、10〜15 行の**スタイル実行カード**を出力し、シナリオライターのプロンプトに直接注入します。
* **チューリングテスト評価ルーブリック**: スタイルプロファイルには `S/A/B/C/D` の 5 段階模倣テスト評価ルーブリックが付属し、人手検証と今後の自動化テストに使用されます（自動化ループはロードマップに計画）。
...
graph TD
    ...
    subgraph "直列分析チェーン"
        ...
        Analyzer1 -- "ストーリー概要を引き渡す" --> Analyzer2[Unified Analyzer 2]

        Chunker --> Block2[テキストブロック 2]
        Block2 --> Analyzer2
        Analyzer2 -- "文脈を引き渡す" --> AnalyzerN[...]

        Chunker --> BlockN[テキストブロック N]
        BlockN --> AnalyzerN
        AnalyzerN --> Synthesis["最終サマリー<br/>標徴的特徴 / 定常 vs 高揚語体 / 作者回避ネガティブ制約 / スタイル実行カード"]
    end

    Synthesis --> FinalProfile[スタイルプロファイル]
    FinalProfile --> Injection["シナリオライターのプロンプトに注入<br/>後続の生成を制約"]
    FinalProfile --> StyleChat["StyleChatAgent<br/>スタイルプロファイル Q&A"]
```
...
* **統一ツールエコシステム**: 56 個のツールがドメイン別にグループ分けされて [registry.py](server/agents/tools/registry.py) に登録され、`agent_tools.py` のパブリックファサードから外部に公開され、エージェントの役割分担に応じてバインドされます。Skill ツールとチャット履歴検索は**条件付き注入**を採用し（Skill がインストール済み / チャットルームが存在する場合にのみマウント）、安定プレフィックスが無関係なツールで汚染されるのを防ぎます。本文、アウトライン、世界設定の部分修正はすべて `_apply_patch` を再利用し、トークン分割やセマンティックチャンキングも共通インフラを利用します。
...
* **実行規律（言った ≠ やった）**: 監督からの委任タスクは、**実際の保存レシート**（`complete_pipeline_step`）をもって完了報告しなければなりません。下書きの出力だけで保存がない場合は差し戻しになります。監督の**進捗ボード**（work_tracker）はタスクリストを永続化し、タスク項目へのバインドを委任時に強制します。同一ツールの連続失敗はサーキットブレーカーをトリガーします。マルチエージェント協働は「帳尻が合い、根拠が残る」ものになります。
* **信頼性基盤**: モデルストリーム空閑ウォッチドッグ（初回アクティブ締切＋単ターンリトライ。ユーザーの待ち時間を増やしません）。チャット / Auto-Write / バックグラウンド構築の 3 種のクラッシュリカバリ（interrupted 状態での進捗保全、カーソルからの再開、二重イベントキャンセル下での部分結果保存）。文脈圧縮時にツールチェーン履歴を分断しない（ToolCall / ToolMessage をペアで保持）。
* **書き前のコンテキスト引き継ぎ**: 監督がシナリオライターに委任する際、**シーン引き継ぎパック**（大綱のシーン契約＋作品メモリプールのリアルタイムな人物状態・関係性・未回収伏線・改訂チケット）を自動構成します。執筆コンテキストは「直近シーンの全文＋チャプターを跨ぐ尾声＋あらすじ / ビートシート」の三層戦略で組み立てられ、長編の一貫性を直接支えます。
...
* **自動テスト**: サーバー側 96 のテストファイル（24 の業務ドメインディレクトリ＋10 の基礎アーキテクチャ契約テスト＋Matchbox ゲートウェイ専用スイート）＋フロントエンド 81 のテストスイート（アーキテクチャ / 性能 / 統合の階層を含む）。基礎アーキテクチャテストでは実際の大規模言語モデルを呼び出すことを禁止し、統一パイプラインのプロトコルのみを守ります。
* **CI 品質ゲート**: i18n の厳密検証（CJK ハードコードスキャンのツールチェーン）、型チェック、フロント / バックエンドの単体テスト、双方のビルド、Docker ビルドを段階的にチェックします。
* **実行時フォレンジック**: オン / オフ切り替え可能なリクエストレベルの JSON 取証（runtime_capture）により、実際のコンテキスト配置とツール呼び出しの閉ループを監査し、キャッシュヒットや品質評価を支援します。
* **コンテンツセーフティ**: 公開共有前の LLM コンプライアンスレビュー（30k 分割の並行審査、不合格なら非公開）；ARC 安全浄化器がモデル入力前に制御命令や挿画プロンプトを除去（プロンプトインジェクション対策）；ツールイベントの機微キーはマスキングされます。
...
業務用データベースは、ワンタッチで PostgreSQL に切り替えることができ、大規模ユーザー環境にも耐える本番級の性能を発揮します。プロジェクト単位のセマンティックベクトル索引はデフォルトで LanceDB を使用し、業務用データベースとは分離されています。
...
2. **起動時の自動アップグレード**: 上流のデータベーススキーマに変更があった場合、起動時に Alembic API が自動で最新の HEAD バージョンへスキーマをアップデートします。既に最新の場合は自動的にスキップします。
3. **最初期段階での実行**: マイグレーションはアプリケーションライフサイクルの最も早い段階で完了し、ビジネス初期化処理がデータベースロックを占有するのを防ぎます。
4. **テンポラリ DB によるスクリプト生成**: マイグレーションスクリプトの自動生成は、移行チェーンからテンポラリ DB を構築して行われるため、開発マシンの実データに汚染されることがなくなりました。
5. **カラム名の変更検知**: カラムのリネームを賢く検出して確認を促します。
6. **危険操作のブロック**: `DROP COLUMN` や `DROP TABLE` などのデータ消失を伴う操作は、実行前に強制的に警告を出し、確認を求めます。
7. **孤児バージョンの自己修復**: 移行履歴が途切れている場合、保守的に足りないテーブルやカラムを追加した上でバージョン番号を合わせます。デフォルトでは余分な構造を削除しません。
8. **head ドリフト保護**: バージョン番号が HEAD に達しているにもかかわらず実カラムが足りない場合はエラーを出し、コミットすべき migration を黙って取りこぼすのを防ぎます。
...
* **運用ループ**: ポイント元帳とモデル別課金、引換コードの発行、クレジット付与キャンペーン（冪等な再付与 / 取り消しセマンティクス）、ユーザーフィードバックチケットとシステム公告——自己デプロイするだけで、完全なマルチテナント運用機能を手に入れられます。
...
* **オプションのローカル埋め込みエンジン**: 外部 API を一切使わないローカルベクトル化——llama.cpp 実行パッケージと GGUF 埋め込みモデルを自動でダウンロード（ミラーフォールバック付き）、ローカルプロセスの管理とヘルス診断まで行い、セットアップ不要で利用できます。
...
パイプラインは **Gitea Actions と GitHub Actions のデュアルエコシステム**で提供されており（デスクトップ / Android リリースパイプラインと Gitee Release 同期を含む）、他の CI へ低コストで移行可能です。
...
> Unity 用 SDK（`SparkArc.Unity`）は、独立したモジュールとして `presenter/UnitySDK` ディレクトリ内に配置されています。個人ゲーム開発者の皆様が、すぐに使えるシナリオソリューションを利用できるように設計されています。**現在は BETA 版です**：ダイアログツリー、分岐ジャンプ、シーン条件評価、状態アーカイブと効果の書き戻し、属性ベースのアクションディスパッチ（Editor アクションマニフェストエクスポータを含む）まで対応済みで、MinimalRuntime サンプルプロジェクト、実行時スモークプローブ、2 本の導入ドキュメントが付属しています。
...
* 稼働中のシステムの利用規約（ToS）画面は、`?lang=` で `LEGAL/TermsOfService.{lang}.md` を優先して読み込み、存在しない場合は `server/data/TermsOfService.md` にフォールバックします。`LEGAL/TermsOfService.zh-CN.md` はサードパーティデプロイ向けの参考テンプレートとしても保持されます。
